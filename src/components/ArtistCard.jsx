...
/**
 * ArtistCard — compact tile, same footprint as a song card cell.
 */
...
export default function ArtistCard({ artist, onOpen }) {
  ...
  const role = artist.role || artist.type || "";
  ...
  return (
    ...
    >
      ...
      <div className="space-y-1 p-4">
        <p className="truncate text-sm font-semibold tracking-tight text-current">{name}</p>
        <p className="truncate text-xs text-current/55">{role || "Tap to explore"}</p>
      </div>
    </button>
  );
}
